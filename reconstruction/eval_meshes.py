...
dataset = data.Shapes3dDataset_Human(
...
if classes == None:
    ...
    folder = ['naive_mad', 'sty_mad', 'human']
else:
...
for idx, item in enumerate(folder):
    generation_dir = os.path.join(out_dir, cfg['generation']['generation_dir'], item)
        ...
        if cfg['test']['eval_pointcloud']:
            ...
            if os.path.exists(pointcloud_file):
                ...
                eval_dict_pcl = evaluator.eval_pointcloud(
                    pointcloud, pointcloud_tgt, normals_tgt=normals_tgt)
    ...
    eval_df_mean = eval_df.groupby(by=['class name']).mean()
    eval_df_std = eval_df.groupby(by=['class name']).std()
    eval_df_class = pd.DataFrame(columns=eval_df_mean.columns)
    for idx in eval_df_mean.index:
        eval_df_class.loc[idx+'_mean'] = eval_df_mean.loc[idx]
        eval_df_class.loc[idx+'_std'] = eval_df_std.loc[idx]
    eval_df_class.loc['all_mean'] = eval_df.mean()
    eval_df_class.loc['all_std'] = eval_df.std()
    eval_df_class.to_csv(out_file_class)

    # Print results
    print('Evaluation Results for %s' % item)
    print(eval_df_class)
