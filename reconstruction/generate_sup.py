...
if classes == None:
    folder = ['naive_mad', 'sty_mad']
elif classes == '03001627':
    folder = ['sty_mad']
else:
    raise ValueError('classes must be 03001627 if it is not null')

for img_folder in folder:
    # Dataset
    dataset = config.get_dataset_test('test', cfg, img_folder, return_idx=True)
    ...
    for it, data in enumerate(tqdm(test_loader)):
        # Output folders
        mesh_dir = os.path.join(generation_dir, img_folder, 'meshes')
        pointcloud_dir = os.path.join(generation_dir, img_folder, 'pointcloud')
        in_dir = os.path.join(generation_dir, img_folder, 'input')
        gt_dir = os.path.join(generation_dir, img_folder, 'groundtruth')
        generation_vis_dir = os.path.join(generation_dir, img_folder,'vis', )
        # Get index etc.
        idx = data['idx'].item()
        ...
        if category_id != 'n/a':
            ...
            pointcloud_dir = os.path.join(pointcloud_dir, str(category_id), modelname)
            in_dir = os.path.join(in_dir, str(category_id), modelname)
            gt_dir = os.path.join(gt_dir, str(category_id), modelname)
        ...
        if generate_pointcloud:
            ...
            for i in range(len(data['inputs'])):
                pointcloud = generator.generate_pointcloud(data, i)
