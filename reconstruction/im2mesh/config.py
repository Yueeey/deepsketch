...
def get_dataset_test(mode, cfg, img_folder, return_idx=False, return_category=False):
    ''' Returns the dataset.
    ...
    if dataset_type == 'Shapes3D':
        ...
        fields = method_dict[method].config.get_data_fields(mode, cfg)
        # Input fields
        inputs_field = get_inputs_field_test(mode, cfg, img_folder)
        if inputs_field is not None:
            fields['inputs'] = inputs_field
...
def get_inputs_field(mode, cfg):
    ...
    if input_type is None:
        ...
        inputs_field = data.ImagesField(
            cfg['data']['img_folder'], cfg['data']['other_folder'], transform,
...
def get_inputs_field_test(mode, cfg, img_folder):
    ''' Returns the inputs fields.
    ...
    if input_type is None:
        ...
        # if not img_folder == 'human':
        #     inputs_field = data.ImagesField(
        #         img_folder, cfg['data']['other_folder'], transform,
        #         with_camera=with_camera, random_view=random_view
        #     )
        # else:
        #     inputs_field = data.ImagesField_Human(
