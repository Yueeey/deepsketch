...
class ImagesField(Field):
    ...
    '''
    def __init__(self, anc_folder, other_folder, transform=None,
                 extension='jpg', random_view=True, with_camera=False):
        # self.anc_folder = anc_folder
        self.anc_folder = 'naive_mad'
        self.other_folder = other_folder
        self.transform = transform
        self.extension = extension
        self.random_view = random_view
        self.with_camera = with_camera
        self.folder_base_name = 'base'
        self.folder_bias_name = 'bias'
    ...
    def load(self, model_path, idx, category):
        ...
        Args:
            ...
            category (int): index of category
        '''
        # anc_folder_path = os.path.join(model_path, self.anc_folder)
        anc_folder_path = os.path.join(model_path, 'sty_mad')
        other_folder_path = os.path.join(model_path,'sty_mad')
        folder_base = os.path.join(anc_folder_path, self.folder_base_name)
        folder_bias = os.path.join(other_folder_path, self.folder_bias_name)
        # files = glob.glob(os.path.join(folder, '*.%s' % self.extension))
        files_base = sorted(glob.glob(os.path.join(folder_base, '*.%s' % self.extension)))
        files_bias = sorted(glob.glob(os.path.join(folder_bias, '*.%s' % self.extension)))
        if self.random_view:
            idx_img = random.randint(0, len(files_base)-1)
            idx_bias = idx_img * 5 + random.randint(0, 4)
        else:
            idx_img = 0
            idx_bias = random.randint(0, 4)

        # filename_base = files_base[idx_img]
        # filename_bias = files_bias[idx_bias]
        # import pudb; pu.db
        ...
        for i in range(5):
            j = 29 + i
            filename_base = files_bias[j]

            image_base = Image.open(filename_base).convert('RGB')
            # image_bias = Image.open(filename_bias).convert('RGB')
            if self.transform is not None:
                image_base = self.transform(image_base)
                # image_bias = self.transform(image_bias)
            
            base.append(image_base)
        ...
        return data, idx_img, idx_bias

    def load_neg(self, model_path, idx, category, idx_img):
        ''' Loads the data point.
        ...
        folder_base = os.path.join(folder_path, self.folder_base_name)
        folder_bias = os.path.join(folder_path, self.folder_bias_name)
        # files = glob.glob(os.path.join(folder, '*.%s' % self.extension))
        files_base = sorted(glob.glob(os.path.join(folder_base, '*.%s' % self.extension)))
        files_bias = sorted(glob.glob(os.path.join(folder_bias, '*.%s' % self.extension)))
        ...
        if self.with_camera:
            camera_file = os.path.join(folder_path, 'cameras.npz')
            camera_dict = np.load(camera_file)
            Rt = camera_dict['world_mat_%d' % idx_img].astype(np.float32)
            K = camera_dict['camera_mat_%d' % idx_img].astype(np.float32)
            data['world_mat'] = Rt
...
#         '''
#         anc_folder_path = os.path.join(model_path, self.anc_folder)
#         other_folder_path = os.path.join(model_path, self.other_folder)
#         folder_base = os.path.join(anc_folder_path, self.folder_base_name)
#         folder_bias = os.path.join(other_folder_path, self.folder_bias_name)
...
#         # image_bias = Image.open(filename_bias).convert('RGB')
#         if self.transform is not None:
#             image_base = self.transform(image)
#             # image_bias = self.transform(image_bias)
